...
# FIXME
isscs(solver) = false # contains(string(typeof(solver)),"SCSSolver")

# Semidefinite solvers
sdp_solvers = MOI.AbstractOptimizer[]
...
mos && push!(sdp_solvers, MathOptInterfaceMosek.MosekOptimizer(QUIET=true))
# Currently, need to create a file param.csdp to avoid printing, see https://github.com/JuliaOpt/CSDP.jl/issues/15
csd && push!(sdp_solvers, CSDP.CSDPOptimizer(printlevel=0))
...
scs && push!(sdp_solvers, SCS.SCSOptimizer(eps=1e-6, max_iters=60000, verbose=0))
