...
function wml19()
    @polyvar x[1:3]
    certificate = Certificate.Newton(SOSCone(), MB.FullBasis{MB.Monomial,typeof(prod(x))}(), tuple())
    ...
    @testset "Example 5.4 $(typeof(ideal_certificate))" for ideal_certificate in
                                                            [
        Certificate.MaxDegree(SOSCone(), MB.FullBasis{MB.Monomial,typeof(prod(x[1:2]))}(), 4),
        Certificate.Newton(SOSCone(), MB.FullBasis{MB.Monomial,typeof(prod(x[1:2]))}(), tuple()),
    ]
...
function l09()
    @polyvar x[1:3]
    certificate = Certificate.Newton(SOSCone(), MB.FullBasis{MB.Monomial,typeof(prod(x[1:2]))}(), tuple())
...
function square_domain(ideal_certificate, d)
    @polyvar x y
    mult_cert = Certificate.MaxDegree(SOSCone(), MB.FullBasis{MB.Monomial,typeof(x * y)}(), d)
    preorder_certificate = Certificate.Putinar(mult_cert, ideal_certificate(typeof(x * y)), d)
    ...
    @testset "Sum square" begin
        ...
        @test set_monos(
            Certificate.Sparsity.sparsity(
                f,
                Sparsity.Variable(),
                Certificate.MaxDegree(SOSCone(), MB.FullBasis{MB.Monomial,typeof(prod(x))}(), 2),
            ),
        ) == expected
...
function drop_monomials()
    @testset "Drop monomials" begin
        @polyvar x
        f = polynomial(x^2)
        certificate = Certificate.MaxDegree(SOSCone(), MB.FullBasis{MB.Monomial,typeof(x^2)}(), 2)
        @testset "$k $use_all_monomials" for k in 0:2,
            use_all_monomials in [false, true]
        ...
        @testset "$(typeof(ideal_certificate))" for ideal_certificate in [
            Certificate.MaxDegree(SOSCone(), MB.FullBasis{MB.Monomial,typeof(x^2)}(), 4),
            Certificate.Newton(SOSCone(), MB.FullBasis{MB.Monomial,typeof(x^2)}(), tuple()),
        ]
            preorder_certificate = Certificate.Putinar(
                Certificate.MaxDegree(SOSCone(), MB.FullBasis{MB.Monomial,typeof(x^2)}(), 3),
                ideal_certificate,
                3,
            )
            f = polynomial(x^3)
...
@testset "Sparsity" begin
    ...
    wml19()
    l09()
    square_domain(M -> Certificate.MaxDegree(SOSCone(), MB.FullBasis{MB.Monomial,M}(), 6), 6)
    square_domain(M -> Certificate.Newton(SOSCone(), MB.FullBasis{MB.Monomial,M}(), tuple()), 6)
    sum_square(8)
