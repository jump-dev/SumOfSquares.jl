@testset "CSP_graph" begin
    @testset "csp_graph" begin
        ...
        G = SumOfSquares.Certificate.Sparsity.csp_graph(MB.SubBasis{MB.Monomial}(MP.monomials(p)), S)
    ...
    @testset "chordal_csp" begin
        ...
        p = x * y + y * z
        S = @set x^2 + y^2 == 1 && y^2 + z^2 == 1
        H, cliques = SumOfSquares.Certificate.Sparsity.chordal_csp_graph(MB.SubBasis{MB.Monomial}(MP.monomials(p)), S)
