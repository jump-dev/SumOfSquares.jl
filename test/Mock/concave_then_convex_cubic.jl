...
optimize_bridged!(mock) = MOIU.mock_optimize!(mock, [1.0; zeros(5); 3.0; zeros(3); 3.0; zeros(7); 3.0; zeros(3); 3.0; zeros(5)],
    (MOI.ScalarAffineFunction{Float64}, MOI.EqualTo{Float64}) => zeros(4),
    (MOI.VectorAffineFunction{Float64}, MOI.Zeros) => [zeros(7), zeros(7)],
    (MOI.VectorOfVariables, MOI.RotatedSecondOrderCone) => [zeros(3) for i in 1:8],
)
optimize_cached!(mock) = MOIU.mock_optimize!(mock, [zeros(2); 3.0; zeros(3); 3.0; zeros(7); 3.0; zeros(3); 3.0; zeros(5); 1.0; zeros(3)],
    (MOI.ScalarAffineFunction{Float64}, MOI.EqualTo{Float64}) => zeros(4),
    (MOI.VectorAffineFunction{Float64}, MOI.Zeros) => [zeros(7), zeros(7)],
    (MOI.VectorOfVariables, MOI.RotatedSecondOrderCone) => [zeros(3) for i in 1:8],
)
...
optimize_bridged!(mock) = MOIU.mock_optimize!(mock, [1.0; zeros(5); 6.0; zeros(8); 6.0; zeros(6)],
    (MOI.ScalarAffineFunction{Float64}, MOI.EqualTo{Float64}) => zeros(4),
...
optimize_cached!(mock) = MOIU.mock_optimize!(mock, [zeros(5); 6.0; zeros(8); 6.0; zeros(3); 1.0; zeros(3)],
    (MOI.ScalarAffineFunction{Float64}, MOI.EqualTo{Float64}) => zeros(4),
    (MOI.VectorOfVariables, MOI.PositiveSemidefiniteConeTriangle) => [zeros(6), zeros(6)],
    (MOI.VectorAffineFunction{Float64}, MOI.Zeros) => [zeros(7), zeros(7)],
    (MOI.VectorOfVariables, MOI.RotatedSecondOrderCone) => [zeros(3), zeros(3)],
