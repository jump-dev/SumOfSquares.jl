...
"""
    simple_matrix(optimizer, config::MOI.Test.Config)
...
function simple_matrix_test(optimizer, ::MOI.Test.Config)
    ...
    @test MB.keys_as_monomials(length(gram_matrix(mat_cref).basis)) == 3
    ...
    @test MB.keys_as_monomials(length(gram_matrix(cref).basis)) == 3
end
