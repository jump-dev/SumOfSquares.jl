...
#using .DihedralGroups.DihedralGroups
...
function SymbolicWedderburn.action(::DihedralAction, el::DihedralGroups.DihedralElement, poly::MP.AbstractPolynomialLike)
    isone(el) && return poly
    x, y = variables(poly)
    if iseven(DihedralGroups.DihedralGroups.isreflection(el) + el.id)
        var_x, var_y = x, y
    else
        var_x, var_y = y, x
    end
    ...
    sign_y = 2 <= el.id ? -1 : 1
    return poly(x => sign_x * var_x, y => sign_y * var_y)
...
G = DihedralGroups.DihedralGroup(4)
for g in G
...
_group(G::DihedralGroup2) = DihedralGroups.DihedralGroup(G.n)
PermutationGroups.gens(G::DihedralGroup2) = [DihedralGroups.DihedralGroups.DihedralGroups.DihedralElement(false, 1, _group(G)), DihedralGroups.DihedralGroups.DihedralGroups.DihedralElement(true, 0, _group(G))]
...
function SymbolicWedderburn.conjugacy_classes(d::DihedralGroup2)
    orbits = [_orbit(DihedralGroups.DihedralGroups.DihedralGroups.DihedralElement(false, 0, _group(d)))]
    for i in 1:div(d.n - 1, 2)
        push!(orbits, _orbit([
            DihedralGroups.DihedralGroups.DihedralGroups.DihedralElement(false, i, _group(d)),
            DihedralGroups.DihedralGroups.DihedralGroups.DihedralElement(false, _group(d).n - i, _group(d)),
        ]))
    end
    if iseven(d.n)
        push!(orbits, _orbit(DihedralGroups.DihedralGroups.DihedralGroups.DihedralElement(false, div(d.n, 2), _group(d))))
        push!(orbits, _orbit([
            DihedralGroups.DihedralGroups.DihedralGroups.DihedralElement(true, i, _group(d)) for i in 0:2:(d.n - 2)
        ]))
        push!(orbits, _orbit([
            DihedralGroups.DihedralGroups.DihedralGroups.DihedralElement(true, i, _group(d)) for i in 1:2:(d.n - 1)
        ]))
    else
        push!(orbits, _orbit([
            DihedralGroups.DihedralGroups.DihedralGroups.DihedralElement(true, i, _group(d)) for i in 0:(d.n - 1)
        ]))
    end
end
